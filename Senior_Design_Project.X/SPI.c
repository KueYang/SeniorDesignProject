...
void SPI2_Init(void)
{
    ...
    SPI2CONbits.MSSEN = 1;      // Slave select SPI support disabled
    ...
    SPI2CONbits.MODE16 = 0b01;  // 16-bit communication
    SPI2CONbits.SMP = 0;        // Input data sampled at middle of data output
    SPI2CONbits.CKE = 0;        // Serial output data changes from idle to active clk
    SPI2CONbits.SSEN = 0;       // Slave select disable
    SPI2CONbits.CKP = 0;        // Clk is active high
    ...
    SPI2BRG = 0;                // SPI clock speed at PBCLK/2
